...
import { ChatCommunitySubInfo, ChatSubInfo, UserNotice } from "twitch-chat-client/lib";
import { TCSEventGiftSubListeningModel, TCSEventResubListeningModel } from "../../../structures/databaseTypes/schemas/TwitchChannelSettings";
...
export default class ConnectedEvent extends BaseEvent {
  ...
  async run(client: TwitchManager, channel: string, username: string, subInfo: ChatSubInfo, userNotice: UserNotice): Promise<void> {
    ...
    if (userNotice.channelId) {
      ...
      const listenSettings = await TCSEventResubListeningModel.findOne({ channelId: userNotice.channelId })
