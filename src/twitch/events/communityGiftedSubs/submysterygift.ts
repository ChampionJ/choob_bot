...
export default class ConnectedEvent extends BaseEvent {
  ...
  async run(
    ...
    ChoobLogger.verbose(
      `There were ${subInfo.count} subs gifted in ${channel} by ${username}`
    );
    ...
    if (userNotice.channelId) {
      ...
      const giftListenSettings = await TCSEventGiftSubListeningModel.findOne({
        channelId: userNotice.channelId,
      });

      if (giftListenSettings?.isListening) {
        const giftChannelOptions = await TCSEventGiftSubOptionsModel.findOne({
          channelId: userNotice.channelId,
        });
        if (
          giftChannelOptions &&
          giftChannelOptions.minimumNumOfSubs <= subInfo.count
        ) {
          setTimeout(async () => {
            // giftQuote: TwitchEventMessageGiftedSubs | null;// = localdata.giftedsubs[0];
            const filteredQuotes = StateManager.giftedSubQuotes.filter(
              (val) => val.minimumGifts <= subInfo.count
            );
            const giftQuote =
            ...
            if (giftQuote) {
              client.sendMsg(
                userNotice.channelId!,
                channel,
                giftQuote!
                  .message!.replace("{gifter}", username)
                  .replace("{number}", subInfo.count.toString())
              );
            } else {
              ChoobLogger.error(
                `Attempted TwitchSubGiftedSubsMessage fetch returned no results!`
              );
            }
