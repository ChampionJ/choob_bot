  ...
  async run(client: TwitchManager, targetChannel: string, message: TwitchPrivateMessage, args: Array<string>): Promise<void> {
    ...
    for (let msgnum = 0; msgnum < StateManager.choobs.length; msgnum++) {
      if (stringSimilarity(StateManager.choobs[msgnum].quote!, newChoob) > 0.8) {
        ...
        ChoobLogger.info(`* Attempted to add duplicate choob quote.\n"${newChoob}"\nmatched\n"${StateManager.choobs[msgnum].quote}"`);
        return;
      }
    }
    ...
    await choob.save().then((choobMessage: mongoose.Document) => {
      ...
      ChoobLogger.info(`${message.userInfo.userName} added ${choobMsg.quote} to choob collection!`)
      ...
      if (err.code !== 11000)
        ChoobLogger.error(`Non-Duplicate error while adding ${newChoob} to database`, err)
      if (err.code === 11000)
        ChoobLogger.error(`Duplicate object error while adding ${newChoob} to database`, err)
    })
  }
}