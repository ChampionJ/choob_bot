...
export default class RemoveChoobCommand extends BaseTwitchCommand {
  constructor() {
    super(
      "removechoob",
  ...
  async run(
    ...
    for (let msgnum = 0; msgnum < StateManager.choobs.length; msgnum++) {
      if (StateManager.choobs[msgnum].quote! === removalChoob) {
    ...
    if (shouldRemoveOne) {
      await ChoobQuoteModel.deleteOne({ quote: removalChoob }).then((res) => {
        if (res.ok === 1) {
          client.sendMsg(
            message.channelId!,
            targetChannel,
            `Removed ${removalChoob} from choob collection!`
          );
        }
      });
    } else {
      client.sendMsg(
        message.channelId!,
        targetChannel,
        `Could not find matching choob ${removalChoob} in the collection to remove!`
      );
    }
  }
}
