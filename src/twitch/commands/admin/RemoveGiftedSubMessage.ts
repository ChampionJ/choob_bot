...
export default class RemoveGiftedSubMessageCommand extends BaseTwitchCommand {
  constructor() {
    super(
      "removegiftmessage",
  ...
  async run(
    ...
    for (
      ...
      if (StateManager.giftedSubQuotes[msgnum].message! === removalMessage) {
        // TODO: this should be a percentage match
        removalIndex = msgnum;
    ...
    if (shouldRemoveOne) {
      await TwitchEventMessageGiftedSubsModel.deleteOne({
        ...
        if (res.ok === 1) {
          client.sendMsg(
            message.channelId!,
            targetChannel,
            `Removed ${removalMessage} from gifted sub message collection!`
          );
          StateManager.emit(
            "twitchGiftedSubsMessageRemoved",
            removalIndex,
            removalMessage
          );
        }
      ...
      client.sendMsg(
        message.channelId!,
        targetChannel,
        `Could not find matching message ${removalMessage} in the collection to remove!`
      );
    }
  }
}
