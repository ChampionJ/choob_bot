...
export default class RemoveChoobManagerCommand extends BaseTwitchCommand {
  constructor() {
    super(
      "removechoobmanager",
      ...
      if (user) {
        ...
        if (!model) {
          client.sendMsg(
            message.channelId!,
            targetChannel,
            `@${message.userInfo.displayName} couldn't find choob user: ${args[0]}`
          );
          return;
        }
        const res = await model.removeRolesAndSave([
          ChoobRole.ADDCHOOB,
        ...
        if (res) {
          if (res?.rolesChanged > 0) {
            client.sendMsg(
              message.channelId!,
              targetChannel,
              `@${user.displayName} can no longer manage choobs!`
            );
            ChoobLogger.info(
              `${message.userInfo.displayName} removed choob manager ${user.displayName}.`
            );
          } else {
            client.sendMsg(
              message.channelId!,
              targetChannel,
              `@${user.displayName} could not manage choobs.`
            );
          }
        } else {
          client.sendMsg(
